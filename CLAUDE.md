...
**音频规格建模戒律（`core/src/model.rs` + `core/src/probe/`）**：⓪ **认不出的编码留空，不拿容器名冒充**——「m4a」是容器，里面可能是 ALAC、AAC 也可能是 APAC，填容器名会让界面言之凿凿地显示一个其实没认出来的编码（`probe_notes` 记 `codec:unrecognized` 留待回溯）；扫描的扩展名列表则宁可收宽，`.ec3` 这类当前放不了的也要收，不收等于让用户以为自己没有这些文件。① 声道**位掩码是权威**，具名 `ChannelLayout` 只是它的投影——6 声道可能是 5.1 也可能是 6.0，摆位不同下混系数就不同，**判不出一律留空，不用声道数硬猜**；② 空间音频（`SpatialFormat`）与声道维度**正交独立**，Atmos 的声道数可能报 5.1 甚至 2；③ `codec` / `container` 存探测器报告的原始名，不归一化（归一化会丢信息）；④ **识别与播放能力解耦**——扫描只如实记录规格，播不了是播放器的事，不能因为暂时播不了就在扫描阶段丢文件；解析失败计入 `failed` 上报，不静默丢弃；⑤ 增强探测逻辑时必须把 `PROBE_VERSION` +1，否则无法识别哪些条目需要重扫，读不懂的线索塞 `probe_notes` 留待回溯。
...
**无缝换曲的三态与两条铁律（`audio/src/ring.rs` + `audio/src/engine.rs` + `src/hooks/useSyncNext.ts`）**：换曲**不拆输出流**——设备重新协商、缓冲重填、位置时基重建，那一整套正是曲目之间那声停顿的来源。于是环形缓冲里会同时躺着两首歌的 PCM，同一时刻就有三种「当前」：`head`（解码头，正在往缓冲里写的那首，可以领先播放一秒半）、`sounding`（**正在发声**的那首）、`staged`（已备好的下一首）。① **切歌由消费端越过边界帧时判定，不是解码器换源时**——按后者发事件，界面会在上一首还在响的时候就换成下一首的标题；同理，进度里的时长与「seek 该定位到哪个文件」都要问 `sounding` 而不是 `head`。② **「下一首」必须与 `Load` 原子下发**（`player_load` 带 `next`），后续变化才走 `player_set_next`：命令跨 IPC 没有顺序保证，而显式装载会清掉待接续的下一首（它属于上一个处境）——晚到的被清、早到的被抹，两种顺序都可能，表现就是「无缝换曲时好时坏」。撤销一份**已经写进缓冲但还没发声**的下一首，走的是与 flush 同形状的「请求 + 回执」协议（消费端收到上限后不再越过，生产端据此安全回退写下标）；作废的打点**不回收**，回退打点下标会与消费端正在读的槽位打架，它留在原地与新打点落在同一个绝对下标上，一起被越过，哪一条该发事件由生产端自己记。前端侧的对账按**队列项 uid**（不是下标递增——随机顺序与拖拽重排会让「下一个下标」失去意义），且 `trackChanged` 是**既成事实**，不因队列版本过期而丢弃，唯一例外是显式装载在途（用户已经另点一首，引擎马上会切过去）。「下一首是谁」由 `useSyncNext` 订阅队列 / 循环 / 随机的变化统一重算，不在十几个队列动作里各写一行——漏掉任何一个的表现都是「偶尔不无缝」。另有一处取舍要知道：**整条输出流的采样率由第一首协商决定**，后面的曲目重采样到它上面（每首按自己的采样率重开设备能省掉这级转换，但换曲必然有停顿，两者不可兼得）。

**换输出端点的四条（`audio/src/output/` + `audio/src/engine.rs` 的 `switch_device` + `src/hooks/useSyncDevice.ts`）**：架构约束不变量第 8 条要求它是**显式状态机**，不许静默改用另一条质量不同的输出。① **端点身份用 cpal 的 `DeviceId`，不用设备名**——同型号的两只 USB DAC 名字一模一样，按名字选会随机打中一只；`DeviceId` 就是为持久化设计的（macOS 上取 CoreAudio 的设备 UID，跨重启与重新插拔都不变），按名字片段匹配只保留给 `play --device` 那条诊断路径。挑不到指定的那台一律**报错而不回落默认**：静默换一台意味着用户以为声音在 DAC 上、实际从笔记本喇叭里出来，而界面还显示着他选的那台。② **先预检再拆流**——`negotiate` 只是预演、不碰设备，所以它失败时旧流仍在正常发声，此时把流拆了等于「点了一台用不了的设备，正在听的歌就停了」；这条失败走 `DeviceRejected` 而**不是** `Error`，后者的含义是播放链已经拆掉，用它报换设备失败会让界面显示成已停止而用户耳朵里音乐还在响。③ **换端点不是新装载**：曲目、队列位置、待接续的下一首都要活过来，所以不能用 `teardown`（那是显式装载的语义，会把 `next_request`、链 ID 与版本一并清掉）。特别注意解码头**可能已经跑到下一首**了——那一步（`advance_head`）早把请求清了，得在丢弃 head 前按 `load_id != sounding.load_id` 判出它是未来那首并退回待接续槽，否则每换一次设备就正好丢掉一次无缝接续，且只在换过设备的那个曲目边界上才看得出来。位置取 **`played_frames`（已发声）而不是已消费**：两者差一个设备延迟（共享模式普遍数十毫秒），按已消费接会把那一截还没进耳朵的音频跳过去。④ **枚举是查询不是命令**——不进引擎那条串行命令队列，也不因此把引擎起起来；每次菜单展开都重新问系统（设备会插拔，缓存一份只会显示已经拔掉的耳机），且**不按能力筛**（筛掉的代价是设备在系统设置里看得见、在这里凭空消失，不如让它给出一句明确的能力错误）。偏好落在 `ui-settings.json` 并**连设备名一起存**：那台设备可能不在了，只存标识的话界面只能显示成「系统默认」，用户既看不出偏好还在，也无从理解为什么下次插回去声音又自己跑过去了；不在时保留偏好但**不下发**（否则每次启动都弹一句他没做过任何操作的「设备不可用」），界面把它标成「（当前不可用）」。设备选择与回执都带单调递增的 `deviceRevision`，快速连点按 latest-wins 对账；菜单分别标出偏好、系统默认与实际输出，三者不能互相冒充。
...
**收藏与歌单（`core/src/collections.rs` + `src/lib/playlists.ts` + `src/store/player.ts`）**：用户自己攒出来的数据，缓存丢了重扫一遍就有，这里丢了就是真没了。五条：① **键全部落到曲目 ID 上**——`album_id` 由含所在目录的归组键哈希而来，改标签、挪文件、重扫都会变（见 `core/src/id.rs`），拿它当收藏的键，用户整理一次音乐文件夹就会发现收藏全没了。所以专辑收藏存的是**成员曲目 ID 的分组快照**，「当前专辑里有任意一首命中任一组」即视为已收藏；分组边界不能丢，否则收藏 A/B/C 后 C 暂时缺失，用可见的 A/B 取消会留下 C 等它回来复活红心。歌手只能用名字（系统里没有比它更稳的标识），改名会落空，这一条无解、如实记下。② **歌单只存曲目 ID，界面上的 `tracks` 是按当前曲库水合出来的子集**，而落盘的 `trackIds` 才是权威——**写回一律以它为准**。当前查不回本体的曲目（外置盘没挂载、重扫时暂时消失）仍留在 `trackIds` 里等文件回来（ID 是内容哈希，自然接上）；拿子集覆盖等于用户改一次歌单名就悄悄删掉几首歌，而他什么都没删。重排时这类曲目沉到末尾——顺序已经没有意义了，但那不是删除它们的理由。③ **歌单 ID 由后端发号**：它要进数据库当主键、还要被收藏表引用，前端另发一个就会出现「界面指向一个库里不存在的歌单」。代价是新建要等一次 IPC，不做乐观更新——摆一张卡片再让它消失，比一开始就没有更难理解。这与红心不同：收藏是连点几十次的动作，新建歌单是一次。④ **收藏与歌单共用一条写入 FIFO 和一个 `collectionsRevision`**：两者互相引用（新建歌单要接着收藏它、删歌单要连带清收藏标记），而对账读的又是同一趟 `collections_load`，分成两条队列时一条的回执会带着另一条尚未落库的旧状态盖回界面；同理这份快照要么一起采纳、要么一起放弃。对账在「读回来的与界面上的一致」时**不换数组**（`samePlaylists`）——每点一次红心都会跑一趟对账，换对象会让歌单页整片卡片重新测量布局，用户正拖着一张卡时那次重测会让它跳一下。⑤ **落盘存时间戳不存「上周更新」**：那句话属于显示层且要随界面语言变，写进数据库等于把一份中文文案钉死在用户数据里；换算见 `updatedLabelOf`，当天改过的仍说「今天更新」，否则显示日期，后端时钟异常退化出的 0 当作「未知」而不是 1970 年。
