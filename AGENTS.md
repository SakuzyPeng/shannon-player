...
**界面设置持久化（`src/lib/settings.ts` + `src/main.tsx`）**：主题、视图、语言与设置页开关跨重启保留，与播放会话共用 `frontend_state.rs` 那条「后端只原子存取一段文本」的通道（槽位是**固定常量**，不接受前端传来的文件名——那等于把应用数据目录的任意路径交给渲染进程）。三条：① **读取放在 `createRoot` 之前**，不是挂载后再补——主题晚一帧应用就是一次白闪，而它恰好发生在每次启动应用的那一刻；代价只是一次本地文件读取，而页面在那之前本来就还在加载 JS。② 因此这里**不需要** `sessionReady` 那样的就绪守卫：落盘的值在首帧之前就进了 store，任何时刻写出去的都已经是用户的设置而不是默认值（StrictMode 的 cleanup 补写因此无害）。③ 每个字段按**当前支持范围**校验，未知值回落默认：语言只认 `LANGUAGES` 那三项——`Language` 类型里还有繁體中文与日本語（词条备好但没进 UI），照单全收就能让一份旧文件或手改把界面切到我们并未对外承诺的语言上；开关按键逐个取并与默认合并，整份替换会让将来新增的开关在旧文件下变成 `undefined`，界面上显示成「既不是开也不是关」。
