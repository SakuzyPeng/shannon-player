...
**自动化断言证明不了「听着对」**，所以另有一份给耳朵的歌单：`make_playlist` 把一首真实音乐转成 13 种格式（无损五种应当听不出差别；44.1k 与 48k 两份无损的对比就是重采样质量；有损那几首听开头结尾有无咔哒；两首 Opus 拖动进度条听落点处有无杂音），`play` 传目录即连续播放。它与 `make_corpus` 的分工是「给耳朵」与「给断言」，两者都不入库。
...
- **`audio/`（crate `shannon-audio`）** —— 播放引擎：解码、PCM 管线、输出后端。同样零 Tauri 依赖（gapless、seek、欠载压测因此能无头 `cargo test`）。当前状态是**立体声路径**：ALAC / AAC / FLAC / MP3 / WAV / AIFF / CAF / Vorbis 经 Symphonia 解码、Opus 经注册进自建 `CodecRegistry` 的 libopus 绑定解码 → 响度增益 → 重采样 → 声道适配 → 无锁 SPSC 环形缓冲 → CPAL 共享输出，含播放 / 暂停 / seek / 音量斜坡 / 响度归一化 / **无缝换曲** / **输出端点切换**。**多声道不走这条路**——下混与空间化都交给系统（见下），平台原生后端尚未接入，当前遇到多声道报明确的路由错误。已接进 Tauri 与前端（命令 + `player://event` 事件桥），播放条走的是引擎上报的真实位置。
...
**定位（seek）要分开验「位置」和「内容」，它们能各自坏掉**（`audio/src/decode.rs` + `audio/tests/format_matrix.rs`）。只断言「报告的位置数字落在请求附近」是不够的：Opus 装在 Matroska / WebM 里实测正是**数字漂亮、音频对不上**——报告 1.0 秒，解出来的与整曲任何一段都不匹配（全局搜索下最小相对误差 1.6~3.9，而两段不相关信号约为 1.41）。三条：① **Opus 的 seek 必须做 80 ms pre-roll**，它的解码器状态跨包延续，落点处那份状态是空的，实测相对误差自信号 RMS 的 78% 起衰减、约 80 ms 才收敛（RFC 7845 §4.2）；做法是先定位拿到落点，再退回落点前 80 ms 重新起解、把中间的帧解出来丢掉。**只给需要的编码加**（`preroll_frames_for`），MP3 二三十毫秒就收敛，无损逐包独立，全都加等于每次 seek 白解一段。② **有的「容器 + 编码」组合的上游 `FormatReader::seek` 不能信**，`reader_seek_is_trustworthy` 是那份名单，命中的走「重开文件 + 向前解码」——线性解码是好的（与 Ogg 版逐样本一致），所以不能因此拒播，坏的只有定位；代价约 1 ms 每秒音频，把「快而错」换成「慢一点而对」。③ 比对时**跳过定位后的头 100 ms 再比**：那一段是解码器状态的收敛期，算进来会把「热身不够」和「拿错了音频」混成同一个数字，而两者的修法完全不同。已知的既有偏差（Vorbis/Ogg 差 1024 帧、FLAC/Matroska 差 3 帧）写成矩阵里的**期望值**而不是放宽一个统一容差——容差只能表达「差不多就行」，真出现回归时它会默默吃掉。

**`OpusHead.pre_skip` 由播放器持有，不能留给 libopus 适配器的内部状态。** 那个适配器的 `reset()` 只复位解码器，不会恢复已经消费掉的 pre-skip：播过一包后 seek 回 0 会漏裁 312 帧；首次解码前直接 seek 到中途又会把这份只属于流开头的裁剪误套过去。`decode.rs` 打开时把它从 OpusHead 取出并在传给适配器的参数里清零，只有实际落点为 0 才重新装填；`opus_seek_does_not_depend_on_decoder_history` 必须一直覆盖「先解一包再定位」这条运行时路径。
