...
**结论：两条路径听不出差别。** 器材为 AirPods（系统空间音频与头部追踪均生效），
比对工具是 `~/code/swift/SpatialCompare`（独立仓库，界面文案刻意中性、技术差异不上
主界面，以免污染判断）。
...
- **头部追踪的专项比对**：上面那次比对在头部追踪生效的状态下进行且未听出差别，
  但没有做「刻意转头看声场是否稳定」的定向测试，因此只能说「未发现异常」，
  不能说「已确认 B 路径的头部追踪与 A 一致」。
