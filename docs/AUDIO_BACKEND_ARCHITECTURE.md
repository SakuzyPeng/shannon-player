...
- 当前版本已交付阶段 0 的立体声播放链、gapless 接续与输出端点切换，并接入 Tauri 与
  前端；浏览器开发环境仍使用 Mock，平台多声道等后续能力仍按本文分阶段实现。
...
2026-08-01 落定：**macOS 上 A（AVPlayer 直通）与 B（PCM 经应用中转）两条路径听感等价**。
AirPods（空间音频与头部追踪均生效）上比对听不出差别，**头部追踪的表现也一致**，即 Apple
没有对原生 Atmos 流另做一份 B 路径拿不到的处理。于是「系统解码 + 应用管线 + 系统空间化」不是拿音质换可控性，
ReplayGain 与音量斜坡留在自有管线里没有代价，本文原先「结论未定之前不得据此描述能力」
的限制就此解除。**限制解除的只是这一条**：空间输出后端仍在阶段 5 未动工，是否为对象
音频也仍由码流的 JOC 标记判定，不看系统标识（验收条件第 7 条）。详见
[macOS 空间音频回放观测笔记](MACOS_SPATIAL_PLAYBACK_NOTES.md)「A 与 B 的听感比对」。
