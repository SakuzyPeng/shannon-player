...
- 设计日期：2026-07-20；2026-07-26 依据 macOS 概念验证与依赖现场核实修订；
  2026-07-28 依据阶段 0 立体声路径的实际落地修订。
...
| 立体声与单声道上混 | 完成（`mix.rs`） |
| 多声道下混、重采样 | **未做**，报明确的能力错误（阶段 1） |
| store 后继算法、queueRevision、事件桥、MockEngine | 未开始 |

验证方式：`cargo test -p shannon-audio`（23 项，无头，语料现生成不入库）与
...
7. **默认输出设备不一定支持源采样率**。实测本机默认设备（一个虚拟声卡）只提供
   24 kHz 与 48 kHz，而曲库主力是 44.1 kHz 的 ALAC——此时阶段 0 的行为是报明确错误
   而非静默变调，但这意味着**重采样不是阶段 1 的锦上添花，而是可用性前提**，
   应排在阶段 1 的最前面。蓝牙耳机常见只报 48 kHz，同理。
