...
- 设计日期：2026-07-20；2026-07-26 依据 macOS 概念验证与依赖现场核实修订；
  ...
  2026-07-29 补响度归一化设计（依据曲库标签覆盖率、解码吞吐与并发实测）。
...
选定 [`ebur128`](https://crates.io/crates/ebur128) 0.1.10（MIT，与 AGPL-3.0-only 兼容）：
...
- API 与现有管线对齐：`add_frames_f32` 直接吃交错 f32，`loudness_global` 出 LUFS，
  `true_peak` 出真峰值（防削顶用），`loudness_global_multiple` 合并多个实例
  ——**这一条正是专辑增益**，各曲分别测量后合并统计，而不是把整张拼起来重算。
...
| 量 | 实测值 |
| --- | --- |
| 单首 ALAC（4 分钟）完整解码 | 约 0.4 秒（单线程独占）／约 1 秒（并行下） |
| 该首的播放时长 | 4 分钟 |
| 预取余量 | **200 到 500 倍** |

余量有两个数量级，因此哪怕机器慢 100 倍（ALAC 降到 2.3x 实时）预取仍然来得及。
**参差不齐在批量方案里体现为「等多久」，在流式预取里只体现为「余量从 500 倍降到
5 倍」，用户什么也感觉不到。**

于是「预取」与「全库分析」合并为同一件事——一个按播放顺序排优先级的分析队列：

- 优先级 = 距当前播放位置的远近；队列变化（切歌、改队列、开随机）时重排；
- **播放当前曲目时顺带测量它**：反正已经在解码，多一遍 K 加权滤波，成本接近零；
- 后台线程池从队首消费，跑完即全库分析完成——它不再是一个需要用户点的按钮，
  只是「队列跑空」这个状态；
- 跳到队列中段不再是特例，那一段大概率早已算过。

### 并发度：固定保守值 + 后台 QoS，不配置也不 benchmark

实测加速比（M4 Pro，8 P 核 + 4 E 核，150 首样本）：
...
4 线程即取得 80% 的可得收益，8 至 12 线程只多 15% 却多占一倍资源——瓶颈是内存带宽
而非核心数，继续堆线程只是发热。

**不影响播放已实测**：8 线程普通优先级满跑分析整个曲库的同时播放 40 秒，
**欠载 0 次**，环形缓冲水位全程钉在 1.51–1.60 秒（高水位即 1.5 秒，说明始终喂满）。
播放只吃实时的 1/500 CPU，外加 1.5 秒缓冲余量，挤不动它。

因此取 `max(2, 物理核 / 4)` 并将线程标记为后台 QoS（macOS `QOS_CLASS_BACKGROUND`
会调度到能效核，Windows `THREAD_MODE_BACKGROUND_BEGIN`，Linux nice + ioprio）。

**不做成用户配置项**：用户答不上「响度分析该用几个线程」——要回答它得同时知道内存
带宽、QoS 调度、电池影响与存储介质。而且这个参数的正确区间宽得离谱（2 到 12 都远远
够用），为一个怎么填都对的参数做界面纯属负担。
...
QoS 就是把判断交给它的正规途径。`audio/examples/bench_decode.rs` 是定这个默认值的
开发期依据，与 `devices`、`play --device` 同类，不进入产品路径。

唯一该给用户的开关是**意图级**而非参数级的：一个「现在全部分析」的显式动作，
对应「插着电、想一次搞定」的场景。用户答得上「我现在想要快」，答不上「请用 8 个线程」。
...
- **专辑增益的分组依据待定**：`loudness_global_multiple` 要求给出「哪些曲目算一张
  专辑」，而「什么是一张专辑」在本项目里是聚合派生的结论（`aggregate` 两遍 + 封面
  指纹合并）。合辑尤其麻烦——一张 116 首横跨 6 个艺人目录的合辑，按专辑增益归一
  是对是错属产品判断，不在本文档裁定。

### 已知不完美

用户从搜索结果直接跳到一首从未听过的曲目时来不及预取，**当次不归一化**；连续播放
全程准确，只有这种随机跳入是第一次不准、第二次起才准。

补救是给装载阶段一个时间预算（如 300 ms）：算完就用，没算完则本次放弃，结果仍存下来
供下次使用。快机器上几乎总能赶上，慢机器上自动退化为「下次才准」——**同一份代码在
参差的机器上给出各自合理的行为，而不是让慢机器的用户等**。
