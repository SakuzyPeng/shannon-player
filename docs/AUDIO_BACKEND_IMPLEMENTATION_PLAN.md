...
- **重采样在声道映射之前**，即按**源**声道数重采样：单声道上混后再重采样等于把同一条
  声道算两遍。将来做多声道下混时顺序要反过来（先把 12 路下混成 2 路再重采样，
  比重采样 12 路便宜得多）。通则是**在声道数少的那一侧做重采样**，而不是记死某个顺序。
...
| 1 | gapless、ReplayGain、设备切换、语料测试；**多声道到立体声的正确下混**（重采样与 stats 已在阶段 0 完成） | 验收条件第 5、6 条的无头测试常态化 |
