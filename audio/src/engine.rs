...
//! 而拆分带来的复杂度（两线程之间的 generation 同步、seek 的三方握手）没有对应收益。
//!
//! 原设计认为 gapless 会逼出这次拆分（next 与 current 并行解码）。**实测不需要**：
//! 环形缓冲有一秒半的余量，交接只是「同一个循环里换个音源接着写」，谁也不必等谁。
//! 唯一的阻塞动作是打开下一首的文件（几毫秒的 I/O），把它挑在缓冲喂满、控制线程本来
//! 就要休息的那一刻做，它就落在缓冲最厚的位置上。
//!
//! ## 三个「当前曲目」，别混作一谈
//!
//! 做 gapless 之后，环形缓冲里会同时躺着两首歌的 PCM，于是同一时刻有三种「当前」：
//!
//! - `head` —— **解码头**，正在往缓冲里写的那首。它可以领先播放一整首。
//! - `sounding` —— **正在发声**的那首。由消费端越过边界时推进。事件盖章、进度里的
//!   时长、以及 seek 该定位到哪个文件，全部以它为准。
//! - `staged` / `next_request` —— 已备好或待打开的下一首。
//!
//! 用 `head` 冒充 `sounding` 会让界面在上一首还在响的时候就换成下一首的标题，
//! 也会让拖动进度条定位到下一首上。
//!
//! ## 能力边界
//!
...
impl Worker {
    ...
    fn advance_head(&mut self) {
        ...
        // 打点槽位用尽。发生条件是「一首歌的最后一秒里反复改队列」（每改一次留下一条
        // 作废打点）或一串比缓冲还短的曲目。此时**必须彻底放弃这次接续**，连请求一起
        // 丢掉——只把音源放回去的话，`has_successor` 会一直为真，于是缓冲放空后既不
        // 交接也不上报播完，播放器就那么静音卡住，而日志里什么都没有。
        // 丢掉之后走 `TrackEnded` 那条老路：前端会显式装载下一首，代价只是一声停顿。
