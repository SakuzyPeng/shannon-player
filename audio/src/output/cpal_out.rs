//! CPAL 共享模式输出后端。
//!
//! 覆盖范围按架构约束划界：**只有共享模式的立体声**（含单声道上混）归这里；
//! **一切多声道**——无论是否带对象元数据——连同独占、直通、空间路由、设备热插拔，
//! 都归各平台的原生实现。多声道整体划出去是因为下混与空间化都依赖布局标签，
//! 而 CPAL 的配置表达不了布局。
//!
//! 两条实现纪律：
//!
...
impl CpalOutput {
    ...
    /// 什么也听不到。没有这个开关，就只能靠改系统设置来二分，而改系统设置会同时
    /// 影响其它应用，是个比问题本身更大的动作。
    ///
    /// 匹配不到时**报错而不是回落到默认设备**：诊断工具一旦允许静默回落，
