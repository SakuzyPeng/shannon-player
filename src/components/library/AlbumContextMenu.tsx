...
interface Props {
  album: Album;
  children: ReactNode;
  onAction?: (key: MessageKey, album: Album) => void;
}

/** 专辑右键菜单：222px、role=menu、方向键循环 + Enter/Esc（由 Radix 提供），危险项用 --danger。 */
export function AlbumContextMenu({ album, children, onAction }: Props) {
  const { t } = useT();
  return (
    <ContextMenu.Root>
        ...
        <ContextMenu.Content className="animate-menu-pop menu-shadow z-40 w-[222px] origin-top-left rounded-[14px] border border-bd bg-srf p-1.5">
          ...
          {ALBUM_MENU.map((mi, i) =>
            mi.kind === "separator" ? (
              ...
              <ContextMenu.Item
                key={i}
                onSelect={() => onAction?.(mi.labelKey!, album)}
                className={cn(
